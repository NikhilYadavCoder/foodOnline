...
def place_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            order.total = total
            order.tax_data = json.dumps(tax_data, default=float)
            order.total_tax = tax
            ...
            return render(request, 'orders/place_order.html', {
                ...
                'rzp_amount':int(Decimal(order.total) * 100),
            })
        else:
            print(form.errors)
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        mail_subject = "Thanks for trusting us — we can't wait for you to try it!"
        ...
        # clear the cart if payment us success
        cart_items.delete()
